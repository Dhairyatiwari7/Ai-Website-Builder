...
const EditorPanel = ({ selectedElement, onUpdate, onClose }: EditorPanelProps) => {
    ...
    if (!values) return;
    const newStyles = { ...values.styles, [style]: value };
                ...
                <div className='grid grid-cols-2 gap-3'>
                    <div>
                        ...
                        <div className='flex items-center gap-2 border border-gray-400 rounded-md p-1'>
                            <input type='color' className='w-6 h-6 cursor-pointer' value={values.styles.backgroundColor === 'rgba(0,0,0,0)' ? '#ffffff' : values.styles.backgroundColor} onChange={(e) => handleStyleChange('backgroundColor', e.target.value)} />
