...
const Sidebar = ({ isMenuOpen, project, setProject, isGenerating, setIsGenerating }: SidebarProps) => {
    ...
    const handleRevisions=async(e:React.FormEvent)=>{
        ...
        try {
            ...
            setIsGenerating(false)
            toast.error(error?.response?.data?.message || error?.message);
            console.log("Error in handle revision:", error);
            clearInterval(interval)
        }
        ...
        <div className="flex flex-col h-full">
                            ...
                            if (isMessage) {
                                ...
                                return (
                                    <div key={msg.id} className={`flex items-center gap-3 ${isUser ? "justify-end" : "justify-start"}`}>
                                        ...
                                        <div className={`max-w-[80%] p-2 px-4 rounded-2xl shadow-sm text-sm mt-5 leading-relaxed ${isUser ? "bg-linear-to-r from-indigo-500 to-indigo-600 text-white rounded-tr-none" :
                                ...
                                return (
                                    <div key={ver.id} className="w-4/5 mx-auto my-2 p-3 rounded-xl bg-gray-800
                ...
                <form className="m-3 relative" onSubmit={handleRevisions}>
                    <div className="flex items-center gap-2">
                        <textarea disabled={isGenerating} onChange={(e)=>setInput(e.target.value)} value={input} className="flex-1 p-3 rounded-xl resize-none text-sm outline-none ring ring-gray-700 
                        focus:ring-indigo-500 bg-gray-800 text-gray-100 placeholder-gray-400 transition-all" rows={4} placeholder="Describe your website or request changes..."/>
